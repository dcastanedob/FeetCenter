...
class ReportesController extends AbstractActionController
{
    ...
    public function accesosAction(){
      ...
      $empleados = \ClinicaempleadoQuery::create()->useEmpleadoQuery()->useEmpleadoaccesoQuery()->filterByIdrol(1,\Criteria::NOT_EQUAL)->endUse()->endUse()->groupBy('idempleado')->find();
    ...
    public function filteraccesosAction(){
      ...
      if($request->isPost()){
          ...
          $logs = \LoginlogQuery::create()->filterByIdempleado($post_data['empleados'],\Criteria::IN)->filterByLoginlogFechainicio(array('min' => $post_data['from'].' 00:00:00', 'max' => $post_data['to'].' 23:59:59'))->orderByLoginlogFechainicio(\Criteria::DESC)->find()->toArray(null,false,\BasePeer::TYPE_FIELDNAME);

          $arr = array();

          foreach ($logs as $key => $value) {

            $empleado = \EmpleadoQuery::create()->findPk($value['idempleado']);

            $geo =  json_decode($value['loginlog_geo'],true);

            $value['loginlog_geo'] = $geo;
            $value['empleado_nombre'] = $empleado->getEmpleadoNombre();
          ...
          return $this->getResponse()->setContent(json_encode(array('logs' => $arr)));
    ...
    public function tablerodecontrolAction(){
        ...
        if($request->isPost()){
           ...
           // var_dump($today_from);
            ...
            if(date_format($to, "m") !=date_format($yesterday, "m")  )
            {
                $yesterday=$to;
                $diff_days--;
            }
...
//                  }
                  ...
                  $visita_detalles =  \VisitadetalleQuery::create()->filterByIdmembresia(NULL, \Criteria::NOT_EQUAL)->useVisitaQuery()->filterByIdempleado($idempleado)->filterByIdclinica($idclinica)->filterByVisitaFechafin(array('min' => $from, 'max' => $to))->filterByVisitaEstatuspago('pagada')->endUse()->find();
                 ...
                 //$pacientes = array(86476);
                  foreach ($pacientes as $idpaciente){
                      ...
                      if($num_visitas > 0){
                         ...
                         // $fecha_copy = new \DateTime($fecha->format('Y-m-d'));
                          $fecha_copy2 = new \DateTime($fecha->format('Y-m-d'));
                          //echo '<pre> Orig';var_dump($fecha);echo '</pre>';
                          //echo '<pre> 30 ';var_dump($fecha_copy->modify('+30 days'));echo '</pre>';
                          //echo '<pre> 45';var_dump($fecha_copy->modify('+15 days'));echo '</pre>';
                          ...
                          if(\VisitaQuery::create()->filterByIdpaciente($idpaciente)->filterByIdempleado($idempleado)->filterByIdclinica($post_data['idclinica'])->filterByVisitaFechafin(array('min' => $fecha_copy2->format('Y-m-d 23:59:59'), 'max' => $fecha_copy2->modify('+45 days')))->filterByVisitaEstatuspago('pagada')->count() >0)

                          {
                              $tmp['tasa_retorno']['45dias']++;

                          }
                           $fecha_copy3 = new \DateTime($fecha->format('Y-m-d'));
                           //echo '<pre>';var_dump($fecha);echo '</pre>';
                          ...
                          if(\VisitaQuery::create()->filterByIdpaciente($idpaciente)->filterByIdempleado($idempleado)->filterByIdclinica($post_data['idclinica'])->filterByVisitaFechafin(array('min' => $fecha_copy3->format('Y-m-d 23:59:59'), 'max' => $fecha_copy3->modify('+60 days')))->filterByVisitaEstatuspago('pagada')->count() >0)
                            {
