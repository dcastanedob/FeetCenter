    ...
    public function nuevorecesoAction(){
     ...
     public function editareventoAction(){
         ...
         if($this->params()->fromQuery('html')){
            ...
            //VALIDAMOS SI ES POSIBLE PONER EL STATUS CANCELADO
            if($entity->getVisitaStatus() != 'cancelo' && isset($value_options['cancelo'])){


              $visita_fecha_30prev = date("Y-m-d H:i",strtotime("-30 minutes",strtotime($entity->getVisitaFechaInicio())));

              $minutes_diff   = floor(abs(strtotime($visita_fecha_30prev) - time()) / 60);

              if($minutes_diff > 30){
                unset($value_options['cancelo']);
              }
...
//                            $empleado_comision->setEmpleadocomisionAcumulado($new_acumulado);
                         ...
                         */
                         ...
                         if($serivicio->getServicioDependencia() == 'cupon'){
                             ...
                             if($paciente_membresia->getPacientemembresiaCuponesdisponibles() == 0){
